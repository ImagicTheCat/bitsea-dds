...
The `timestamp` should be set at the https://en.wikipedia.org/wiki/Unix_time[Unix time] date of modification. Newer versions of a pointer should replace the current one if their `timestamp` is strictly greater and not into the future. Because time can vary from a system to another, a _seaport_ may use an error margin for the latter comparison.

NOTE: By updating if strictly greater, the pointer is stable for a specific `timestamp` once acquired. By rejecting a `timestamp` too far into the future, it is more consistent with the idea of a pointer; it will be impossible to share a forever immutable pointer.

WARNING: The `timestamp` is not a reliable time source about when the pointer has been modified. Its sole purpose is versioning.
...
A data structure is built on blocks, generally by forming a Hash DAG, where a root block represents the structured resource.
...
A resource of the system is equivalent to a specific block, which can represent a data structure or may be interpreted as a topic.
...
block-v1 = "d" <encoding of the block identifier v1 hash, without prefix>
identity-v1 = "p" <encoding of the identity identifier v1 hash, without prefix>
----

The human-readable encoding of the binary identifiers is based on `base128btc`. The alphabet is specifically `123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz`.

To encode the bytes, they are interpreted as a base-256 big-endian unsigned integer which is then converted to the corresponding base-58 big-endian unsigned integer without padding.

To decode the base-58 payload, it is interpreted as a base-58 big-endian unsigned integer which is then converted to the corresponding base-256 big-endian unsigned integer, with left padding to fit into the expected number of bytes.

NOTE: The incentives for `base128btc` is human-readability and composition: short strings, less ambiguity, no special characters.
...
The semantic of non-root segments is defined by the kind and data of the previously resolved block.

.Examples
block:: `bitsea:dCMn1nKMPqjFA2PxcA2vihGyhTZ9vPjDhGu7hHZVj464H/...`
pointer:: `bitsea:pJx4T3yQnDbUsNRkTzoH9YJz9hAqSsA9JyFu3PA5f7o4P/...`
bound pointer:: `bitsea:pJx4T3yQnDbUsNRkTzoH9YJz9hAqSsA9JyFu3PA5f7o4P-dCMn1nKMPqjFA2PxcA2vihGyhTZ9vPjDhGu7hHZVj464H/...`
